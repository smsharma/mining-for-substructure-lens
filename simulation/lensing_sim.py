...
class LensingSim():
    ...
    def set_up_observation(self):
        ...
        self.x_coords, self.y_coords = np.meshgrid(np.linspace(self.xlims[0], self.xlims[1], self.nx),
    ...
    def lensed_image(self):
        ...
        for lens_dict in self.lenses_list:
            if lens_dict['profile'] == 'sis':
                ...
                _xg, _yg = deflection_sis(self.x_coords, self.y_coords, x0=self.theta_x_hst, y0=self.theta_y_hst,
                                          b=self.theta_E_hst)
                xg += _xg
                yg += _yg
                ...
                _xg, _yg = deflection_nfw(self.x_coords, self.y_coords, x0=self.theta_x_sub, y0=self.theta_y_sub,
                                          M=self.M_sub, D_s=self.D_s, D_l=self.D_l)
                xg += _xg
                yg += _yg
        ...
        for source_dict in self.sources_list:
            if source_dict['profile'] == 'sersic':
                ...
                self.i_lens += f_gal_sersic(self.x_coords - xg, self.y_coords - yg, self.n_srsc, self.I_gal,
                                            self.theta_e_gal)
            else:
