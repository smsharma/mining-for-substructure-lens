...
With the likelihood ratio in hand, a Bayesian interpretation is easily admitted using Eq.~\ref{eq:bayesian_post}. In Fig.~\ref{fig:bayesian_post} we show the derived posteriors, assuming either a uniform prior between $-1.5$ and $-0.5$ (left panel) or a Gaussian prior with mean $-0.9$ and standard deviation $0.1$ (right panel) on the slope $\beta$. The latter is intended to capture a prior expectation on the subhalo mass function slope consistent with the Cold Dark Matter scenario~\citep{0809.0898,0802.2265} and results in a posterior density that is more peaked around the true point.
...
\caption{Expected posterior based on a uniform prior (left) or a Gaussian prior on $\beta$ with mean $-0.9$ and standard deviation $0.1$ (right) for 10 observed lenses. We assume $\beta = -0.9$ and $\fsub = 0.05$. \JB{So far without calibration.}}
...
For this proof-of-concept analysis our lensing simulation makes a number of simplifying assumptions. We will now discuss how our simulation and inference method can be extended to a realistic state-of-the-art model of strong lensing, and how these changes will affect the results.

First, we currently fix all properties of the background source. It is straightforward to instead draw the mass, profile, and redshift from suitable distributions. The most realistic description might be given by generative models trained on real galaxies, which we could easily use within our code.
