...
\caption{Four simulated lens images (upper panels) and the corresponding estimated likelihood maps (lower panels). The star marks the true point used to generate the images, the black line shows $95 \%$~CL contours in parameter space based on each image. \JB{Noise comes from limited statistics in calibration. Should we a) show uncalibrated images here, b) explain the noise, or c) pass everything through a GP to make it more visually appealing?}}
...
We find that, at least within the simplifying assumptions of our simulator, an analysis of more than $\mathcal{O}(10)$ lenses is quite sensitive to the overall substructure abundance parameterized by $\fsub$. Approximately 100 lens images are required to resolve $\beta$. Note the ``banana'' shape of the expected exclusion limits, which approximately traces the deflection due to the substructure as given in Eq.~\eqref{eq:nfw_deflection}.

With the likelihood ratio in hand, a Bayesian interpretation is easily admitted using Eq.~\eqref{eq:bayesian_post}. In Fig.~\ref{fig:bayesian_post} we show the derived posteriors, assuming either a uniform prior between $-1.5$ and $-0.5$ (left panel) or a Gaussian prior with mean $-0.9$ and standard deviation $0.1$ (right panel) on the slope $\beta$. The latter is intended to capture a prior expectation on the subhalo mass function slope consistent with the Cold Dark Matter scenario~\citep{0809.0898,0802.2265} and results in a posterior density that is more peaked around the true point.
...
For the proof-of-concept analysis presented here our lensing simulation makes a number of simplifying assumptions in order to highlight the broad methodological points relevant to the method at hand in a computationally tractable setting. We will now discuss how our simulation and inference method can be extended to a realistic state-of-the-art model of strong lensing, and how these changes will affect the results and details of the analysis procedure.
...
\caption{Expected posterior based on a uniform prior (left) or a Gaussian prior on $\beta$ with mean $-0.9$ and standard deviation $0.1$ (right) for 100 observed lenses. The lines show a $95\%$ credible interval. We assume $\beta = -0.9$ and $\fsub = 0.05$.}
\label{fig:bayesian_post}
\end{figure*}

First, we currently fix all properties of the background source as described in Sec.~\ref{sec:source}. It is straightforward to instead draw and marginalize over the parameters associated with a chosen source light distribution parameterization, with Gaussian and S\'{e}rsic~\citep{1963BAAA....6...41S} profile models being common choices. For high-fidelity images (such as those obtained by targeted followups or interferometric imaging) more complicated features in the background galaxies such as outflows and obscured regions may not be adequately captured by such a parameterization and could introduce degeneracies with the effects of substructure. Alternative parameterizations using shapelet basis sets~\citep{1803.09746, 1504.07629}, and methods based on regularized linear inversion on grids~\citep{1708.07377, astro-ph/0601493,2003ApJ...590..673W} have been introduced as ways to model more complicated source features. For our application, generative/data-driven modeling of background galaxies could be interfaced with our method to account for the variation in structure of the background source~\citep{1901.01359}.

Similarly, the lens properties can be made more realistic by relaxing the restriction to spherical host halos and including more complicated profiles. A realistic simulator should also model the tidal stripping of subhalos, which entails drawing the positions of the subhalos in three dimensions rather than just in two. We can also add external shear. Our subhalo mass function in Eq.~\eqref{eq:shmf} is independent of the lens redshift, but can easily be extended to include this dependency.

All of these effects are straightforward to implement in our setup. They only require modifications to the simulation code. The inference algorithm is unaffected: since these extensions do not explicitly depend on the parameters of interest, the likelihood terms associated with them cancel in the calculation of the joint likelihood ratio and the joint score. Nevertheless, these changes affect the final observed image and therefore also the true likelihood function; the variance of the joint likelihood ratio and score could therefore increase, requiring larger training samples before the network converges to the correct likelihood ratio function.

With these extensions, the redshift of the background source and the lens will play a more important role. Since these redshifts are observable, it is likely that we can improve the performance of the inference algorithm by using this information. We can treat both the source and lens redshift, potentially with added noise to model imperfect observations, as additional observables. The input to the neural networks then consists of the observed lens image, the measured redshifts, and the tested parameter point. Except for a simple modification of the network architecture, the inference algorithm remains unchanged.

Including line-of-sight substructure is somewhat more involved. Its mass function depends on the parameters of interest, which requires a modification of the calculation of the joint likelihood ratio and joint score. Structurally this is identical to our current modeling of subhalos within the lens. Since the number of halos along the line of sight is expected to be substantially smaller than the number of subhalos, we expect that these additional factors in the joint likelihood ratio and joint score will not slow down the overall simulation significantly, and will not increase the variance of the inference techniques too much. In fact, since the line-of-sight substructure is sensitive to the parameters of interest, it is part of the signal rather than a nuisance and has the potential to improve the sensitivity of the measurement to $\fsub$ and $\beta$.

Realistic observations do not have a uniform exposure and PSF over all lenses. These effects can be modeled in the simulator. In this case we envision a normalization of the network input to a unit exposure. Providing the exposure and parameters of the PSF as additional inputs to the network is likely to improve performance in this case. Multiple passbands can also be modeled, the ResNet architecture is commonly used with images with three color channels and can be easily extended to a larger number.

While the implementation of these extensions in our simulation and inference code is feasible, estimating the sensitivity of such a realistic analysis is beyond the scope of this paper. While most of the changes are likely to reduce the information that can be extracted from the lens image, providing the redshifts as additional input, using multiple passbands, and line-of-sight substructure sensitive to the same parameters might in fact provide additional information.
...
The expected confidence limits for 10 observed lens images in the three simplified scenarios and our ``full'' setup are compared in Fig.~\ref{fig:scenarios}. As expected, the more latent variables we keep fixed, the more the inference technique becomes more sensitive to the parameters of interest. In particular fixing the source-host alignment substantially increases the strength of the expected limits.
