...
\JB{Expand this paragraph with a broader statement about NN-based LFI}
...
\caption{Expected posterior and $95\%$ credible regions for 100 observed lenses. The mock observations are generated for $\fsub = 0.05$ and $\beta -0.9$. In the left panel we assume a uniform prior, while the right panel shows results for a uniform prior on $\fsub$ and a Gaussian prior with mean $-0.9$ and standard deviation $0.1$ for $\beta$.}
