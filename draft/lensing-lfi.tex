...
The individual likelihood estimates can be combined in a straightforward manner into a stacked test statistic, yielding a combined analysis of an ensemble of lenses. The expected likelihood surface per-image in the asymptotic limit is shown in the left panel of Fig.~\ref{fig:expected_likelihood}, with the 1-D slice corresponding to $\beta = -0.9$ shown in the right panel. The 95\% CL expected exclusion limits for 1, 5, 20, and 100 lenses are shown using the dotted, dash-dotted, dashed, and solid lines respectively. A larger observed lens sample provides a tighter constraint on substructure properties.
